...
-- {{{ Key bindings
globalkeys = gears.table.join( -- Focus
-- Focus rightawful.key({modkey}, "h", function()
    awful.client.focus.global_bydirection("left")
end, {description = "Focus right", group = "Focus"}), -- Focus left
awful.key({modkey}, "l",
          function() awful.client.focus.global_bydirection("right") end,
          {description = "Focus left", group = "Focus"}), -- Focus below
awful.key({modkey}, "j",
          function() awful.client.focus.global_bydirection("down") end,
          {description = "Focus below", group = "Focus"}), -- Focus above
awful.key({modkey}, "k",
          function() awful.client.focus.global_bydirection("up") end,
          {description = "Focus above", group = "Focus"}), -- Focus urgent
...
-- Cycle with mod+tab
              awful.key({modkey}, "Tab",
                        ...
                        {description = "Focus next", group = "Focus"}),

              awful.key({modkey, "Shift"}, "Tab",
                        function() awful.client.focus.byidx(-1) end,
                        {description = "Focus previous", group = "Focus"}),

-- Focus previous window (back-n-forth)
              awful.key({modkey}, "`", function()
    awful.client.focus.history.previous()
    if client.focus then client.focus:raise() end
end, {description = "go back", group = "Focus"}), -- Switch layout tiling/tab
awful.key({modkey}, "space", function() awful.layout.inc(1) end,
          {description = "Next layout", group = "Layout"}), -- Master window
...
-- Stack windows
              ...
              awful.key({modkey}, "-",
                        function() awful.tag.incncol(-1, nil, true) end,
                        {description = "Substract columns", group = "Layout"}),

-- Standard programs
              awful.key({modkey}, "Return",
                        function() awful.spawn(terminal) end,
                        {description = "Open terminal", group = "Other"}),
...
end, {description = "Screenshot to Clipboard", group = "Other"}), -- Prompt
awful.key({modkey}, "r",
          function() awful.screen.focused().mypromptbox:run() end,
          {description = "Run command", group = "Other"}),

              awful.key({modkey}, "p", function() menubar.show() end,
                        {description = "Run application", group = "Other"}))

clientkeys = gears.table.join( -- Fullscreen and floating toggle
...
for i = 1, 9 do
    ...
    end, {description = "view tag #" .. i, group = "Workspaces"}),
    -- Toggle tag display.
    ...
    end, {description = "toggle tag #" .. i, group = "Workspaces"}),
    -- Move client to tag.
